...
export default function SupplierCard({ supplier, isSelected, onCompareToggle, onQuickView }: SupplierCardProps) {
  const verificationTier = supplier.verificationTier ?? (supplier as any).verification_tier ?? 'bronze';
  const availabilityKey =
    supplier.availability ?? (supplier as any).availability ?? supplier.products?.[0]?.availability ?? 'contact';
  const availability = availabilityConfig[availabilityKey as keyof typeof availabilityConfig];

  const pricePerUnit =
    supplier.pricePerUnit ??
    (supplier as any).price_per_unit ??
    supplier.products?.[0]?.price_per_unit ??
    0;

  const reviewCount = supplier.reviewCount ?? (supplier as any).review_count;
      ...
      >
        ...
        <div className="relative p-6 pt-14">
          <div className="flex flex-col items-center text-center mb-5">
            ...
            <h3
              className="font-serif font-semibold text-lg mb-1 group-hover:text-gold transition-colors duration-300"
          ...
          <div className="flex justify-center mb-5">
            <StarRating rating={supplier.rating} reviewCount={reviewCount} size="sm" />
          </div>
